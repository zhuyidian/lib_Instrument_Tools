...
public class VolumeImpl extends AbsAbility {
    ...
    public void handleMessage(RemoteCommand command) {
        LogUtil.i("time", "---handle message--- command=" + command + ", Thread=" + Thread.currentThread());
    ...
    public void realMessage(RemoteCommand command) {
        LogUtil.i("time", "---real message--- command=" + command + ", Thread=" + Thread.currentThread());
    ...
    private void dealVolume(Context mContext, RemoteCommand command, Volume volume) {
        ...
        IAudio iAudio = SAL.getModule(mContext, SalModule.AUDIO);
        int currentVolume = 0;
        try {
        ...
        boolean result = setVolume(iAudio, convertVolume);
    ...
    private boolean setVolume(IAudio iAudio, int volume) {
        try {
            iAudio.setVolume(volume);
            return true;
        } catch (Exception e) {
            e.printStackTrace();
            LogUtil.e("time", "iAudio=" + iAudio + ", e=" + e);
        }

        return false;

        //不需要再调用系统API了，全部交给SAL处理
//        try {
//            LogUtil.i("time", "set volume volume=" + volume);
//            AudioManager manager = (AudioManager) mContext.getSystemService(Context.AUDIO_SERVICE);
//            manager.setStreamVolume(AudioManager.STREAM_MUSIC, volume, AudioManager.FLAG_SHOW_UI | AudioManager.FLAG_VIBRATE);
//        } catch (Exception e) {
//            e.printStackTrace();
//            LogUtil.e("time", "e1=" + e);
//        }
    ...
    private int convertVolume(int targetVolume, int maxValue) {
        ...
        LogUtil.i("time", "targetVolume=" + targetVolume + ", convertVom=" + convertVom + ", target=" + target);
