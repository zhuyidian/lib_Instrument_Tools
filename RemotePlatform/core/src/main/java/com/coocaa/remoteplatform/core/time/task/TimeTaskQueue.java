...
public class TimeTaskQueue {
    ...
    public boolean addTimeTask(int cmdType, TimeBean bean, RemoteCommand command, IMessageDispatcher dispatcher) {
        ...
        TimeTaskBean timeTaskBean = new TimeTaskBean(cmdType, bean, command, dispatcher);
        ...
        LogUtil.i("time", "add time task success cmdType=" + cmdType + ", planType=" + bean.getPlanType() +
                ", exe time=" + TimeAnalysis.long2string(timeLong) + ", queue size=" + timeTaskMap.size());
        return true;
    }
    ...
    public TimeTaskBean getNearestTimeTask() {
        try {
            ...
            while (it1.hasNext()) {
                ...
                if (timeTaskBean == null) continue;
                if (timeTaskBean.getBean() == null) continue;
                if (timeTaskBean.getTime() == 0l) continue;
                list.add(timeTaskBean);
            }

            //if (list == null || list.isEmpty()) return null;
            sortNearestTime(list);
    ...
    private void sortNearestTime(List<TimeTaskBean> list) {
        try {
            Iterator<TimeTaskBean> iter1 = list.iterator();
            while (iter1.hasNext()) {
                TimeTaskBean timeTaskBean = (TimeTaskBean) iter1.next();
                LogUtil.i("time", "sort before cmdType=" + timeTaskBean.getCmdType() + ", planType=" + timeTaskBean.getBean().getPlanType() + ", timeLong=" + timeTaskBean.getTime() + ", timeStr=" + TimeAnalysis.long2string(timeTaskBean.getTime()));
            }
            ...
            while (iter2.hasNext()) {
                ...
                LogUtil.i("time", "sort after cmdType=" + timeTaskBean.getCmdType() + ", planType=" + timeTaskBean.getBean().getPlanType() + ", timeLong=" + timeTaskBean.getTime() + ", timeStr=" + TimeAnalysis.long2string(timeTaskBean.getTime()));
            }
        } catch (Exception e) {
            e.printStackTrace();
        }
