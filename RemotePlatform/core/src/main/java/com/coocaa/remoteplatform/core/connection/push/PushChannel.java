...
import com.coocaa.remoteplatform.core.http.HttpResponseData;
import com.coocaa.remoteplatform.core.http.HttpService;
...
public class PushChannel implements IConnectChannel {
    ...
    private final BroadcastReceiver attachInfoChangeReceiver = new BroadcastReceiver() {
        ...
        public void onReceive(Context context, Intent intent) {
            ...
            Log.i(TAG, "onReceive: which=" + which + ", newActiveId=" + newActiveId);
    ...
    RemoteCommand convertPushCommandToCommand(PushCommand pushCommand) {
        ...
        try {
            ...
            result.msgId = pushCommand.msgId;
    ...
    public boolean replyMessage(RemoteCommand command) {
        HttpService.getInstance(mContext).reportPushResult(command.msgId, mAttachInfo.getDeviceId(), mAttachInfo.getMac(), Integer.parseInt(command.status), command.msg, "", new Callback<Response<HttpResponseData>>() {
            @Override
            public void onResponse(Call<Response<HttpResponseData>> call, Response<Response<HttpResponseData>> response) {

            }

            @Override
            public void onFailure(Call<Response<HttpResponseData>> call, Throwable throwable) {
