...
import static com.coocaa.remoteplatform.core.common.Constant.REMOTE_METHOD_SUBSCRIBE_TOPIC;
import static com.coocaa.remoteplatform.core.common.Constant.REMOTE_METHOD_UN_SUBSCRIBE_TOPIC;
...
public class RemotePlatform implements IRemotePlatform {
    ...
    private IPCSocketClient mIPCSocketClient = null;
    ...
    @Override
    public void subScribeTopic(String topic, IIPCSocketReceiver callback) {
        createTopicHandler();
        if (!checkService()) {
            return;
        }
        mIPCSocketClient.subScribeTopic(topic, callback);
        Map<String, String> params = new HashMap<>(1);
        params.put(Constant.REMOTE_METHOD_PARAMS_TOPIC, topic);
        try {
            mService.invokeMethod(REMOTE_METHOD_SUBSCRIBE_TOPIC, params);
        } catch (RemoteException e) {
            e.printStackTrace();
        }
    }
    ...
    public void unSubScribeTopic(String topic) {
        if (mIPCSocketClient == null) {
            return;
        }
        if (!checkService()) {
            return;
        }
        mIPCSocketClient.unSubscribeTopic(topic);
        Map<String, String> params = new HashMap<>(1);
        params.put(Constant.REMOTE_METHOD_PARAMS_TOPIC, topic);
        try {
            mService.invokeMethod(REMOTE_METHOD_UN_SUBSCRIBE_TOPIC, params);
        } catch (RemoteException e) {
            e.printStackTrace();
        }
    }

    private void createTopicHandler() {
        if (mIPCSocketClient == null) {
            mIPCSocketClient = new IPCSocketClient();
        }
    }
    ...
    public void sendTopicMessage(String method, String topic, String message) {
        if (mIPCSocketClient != null) {
            mIPCSocketClient.enqueueMessage(method, topic, message);
        }
    }
